...
from flex_fab_agent.eval.runner import _evaluate_single_case
...
def test_evaluate_single_case_three_layers(monkeypatch):
    ...
    from flex_fab_agent.eval import runner as runner_mod
...
def test_evaluate_single_case_multi_returns_trajectory(monkeypatch):
    ...
    from flex_fab_agent.agents import supervisor as supervisor_mod
...
def test_run_eval_disables_semantic_cache(monkeypatch):
    """① eval runner 默认禁用语义缓存（避免陈旧缓存回放污染评估）。

    测试后须还原 SEMANTIC_CACHE（run_eval 直接改 os.environ，单进程 pytest 共享
    env，否则污染 test_is_semantic_enabled_default）。
    """
    import os
    from flex_fab_agent.eval import runner as runner_mod
    ...
    try:
        ...
        monkeypatch.setattr(runner_mod, "load_cases", lambda: [])
        runner_mod.run_eval()
        assert os.environ.get("SEMANTIC_CACHE") == "off", "eval 必须默认禁语义缓存"
    finally:
        if orig is None:
            os.environ.pop("SEMANTIC_CACHE", None)
        else:
