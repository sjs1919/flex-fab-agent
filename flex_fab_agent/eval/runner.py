...
用法：
  python -m flex_fab_agent.eval.runner           # 跑全部（单 Agent 模式，含 judge）
  python -m flex_fab_agent.eval.runner --case eval_001  # 跑单个
  python -m flex_fab_agent.eval.runner --mode multi      # 多 Agent 模式
  python -m flex_fab_agent.eval.runner --no-judge        # 跳过 LLM-as-Judge（评估提速/省钱）
  python -m flex_fab_agent.eval.runner --report          # 生成 HTML 报告
"""
import json
import os
import sys
...
def main():
    """CLI 入口（python -m flex_fab_agent.eval.runner）。"""
    import argparse
