...
python -m flex_fab_agent.main --check

# 2. 单 Agent 问一个问题（v1 模式）
python -m flex_fab_agent.main "今天先做哪些订单？"

# 3. 多轮对话（状态持久化，重启可恢复）
python -m flex_fab_agent.main --chat
...
# 6. 求解一轮排产并落库（M2 + M4a）
python -m flex_fab_agent.main --init-schedule

# 7. 启动生产模拟器心跳（M3 + M4a，Ctrl+C 停止）
python -m flex_fab_agent.main --sim

# 8. Prompt 版本回滚（R-4 安全回滚）
python -m flex_fab_agent.main --rollback v1
...
python -m flex_fab_agent.auth.mint admin
...
| 单一问题快速回答 | `python -m flex_fab_agent.main "问题"` | 单 Agent 足够，链路短、token 省 |
...
**CLI 直接调用**：
```bash
python -m flex_fab_agent.scheduler.solver --solve --out result.json
```
...
python -m flex_fab_agent.eval.runner --case eval_001   # 单个 case
python -m flex_fab_agent.eval.runner --no-judge        # 跳过 LLM-as-Judge

# RAGAS 四指标回归（需真实 LLM，Q1-Q5）
python -m flex_fab_agent.eval.ragas_regression
...
### v1 能力（已全部验收）
- [x] `python -m flex_fab_agent.main --check` 地基自检通过
