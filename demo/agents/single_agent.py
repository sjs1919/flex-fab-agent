...
# 状态/时效敏感查询关键词：命中即标记 sensitive（短 TTL + 数据变更主动清除，
# 避免 9:00 与 10:00 问同一订单返回过时答案）。宁可漏缓存，不可缓存过时答案。
# 只收强状态词（发现③）：曾含「排产/订单/库存/设备/查询/多少」等 ~20 个高频词，
# 「什么是排产」这类知识类问题被大面积误标 sensitive，语义缓存对领域知识基本失效。
_STATE_SENSITIVE_KEYWORDS = (
    "状态", "进度", "排队", "当前", "还剩", "有没有",
    # 2026-08-29 补充状态口吻引导词：仅靠上面 6 词会漏判「目前待排产的订单」
    # 这类口语查询（无强状态词），失败答案被当知识类固化 24h。只补查询引导词，
    # 不补「排产/订单」领域词（避免「什么是排产」类知识问题误标 sensitive）。
    "目前", "还有", "剩",
)
