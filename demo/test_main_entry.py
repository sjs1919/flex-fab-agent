...
def _run_main(monkeypatch, argv):
    monkeypatch.setattr("sys.argv", ["demo.main"] + argv)
    import demo.main as m
    m.main()


def test_check_prints_tool_count(monkeypatch, capsys):
    monkeypatch.setattr("demo.main.available_providers", lambda: [{"name": "fake"}])
    monkeypatch.setattr("demo.main.get_data_source", lambda: "csv")
    monkeypatch.setattr("demo.main.load_orders", lambda: [])
    import demo.core.llm_client as lc
