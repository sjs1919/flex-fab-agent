...
from demo.config import available_providers, get_data_source, CREDENTIALS_FILE  # noqa: E402
...
def s1_foundation() -> None:
    ...
    # S1.2 工具注册表
    try:
        from demo.tools.registry import build_default_registry
    ...
    # S1.3 数据加载
    try:
        from demo.tools.data import load_orders
        orders = load_orders()
...
def s2_data_layer() -> None:
    ...
    if ds == "mysql":
        try:
            from demo.schema.migrate import status
            s = status()
            ...
            if max(versions or [0]) >= 3 and s["table_count"] >= 18:
                ...
                _fail("S2.1", "MySQL 迁移状态", f"version={max(versions or [0])}, tables={s['table_count']}; 请执行 python -m demo.schema.migrate --up")
        except Exception as e:
...
def s3_data_integrity() -> None:
    ...
    try:
        from demo.tools.data import load_orders, load_machines, load_parts, load_customers
        orders = load_orders()
