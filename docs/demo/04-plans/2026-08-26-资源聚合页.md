...
运行（WSL venv）：`.venv/bin/python -m pytest demo/test_resources.py -v`
...
运行：`.venv/bin/python -m pytest demo/test_resources.py -v`
预期：3 个测试全 PASS（items 非空 + id 倒序 + 404）

- [ ] **步骤 5：Commit**

```bash
git add demo/api.py demo/test_resources.py
