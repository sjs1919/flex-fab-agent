...
> **导入路径关键决策（已验证）**：`eval/judge.py`、`eval/runner.py` 内部用相对导入（`from ..core.llm_client import call_llm`），这要求 `flex-fab-agent` 必须是包（有 `__init__.py`）。因此 conftest 必须把 **agent-training 根**（flex-fab-agent 的父目录）加入 sys.path，测试统一用 `from demo.eval.xxx import ...`。已验证 `python -c "from demo.eval.runner import run_eval"` 在 agent-training 根下成功。
...
> 说明：`demo/pytest.ini` 的 `testpaths = eval tests` 用相对路径，pytest 从 flex-fab-agent 目录启动时自动定位。conftest 放在 flex-fab-agent 根，pytest 收集 eval/ 下测试时自动加载。
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m demo.eval.runner --no-judge`
...
- 测试文件 import 路径统一为 `from demo.eval.xxx import ...`（conftest 把 flex-fab-agent 根加入 sys.path）
