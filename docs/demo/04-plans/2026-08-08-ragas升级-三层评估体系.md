# flex-fab-agent Agent 评估体系改造实现计划

> **面向 AI 代理的工作者：** 必需子技能：使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans 逐任务实现此计划。步骤使用复选框（`- [ ]`）语法来跟踪进度。

**目标：** 将 flex-fab-agent 的手写 eval 升级为三层评估体系：保留工具层指标 + 新增自研 LLM-as-Judge 语义指标 + 新增轨迹评估（路径效率/重试分析）+ 单页可视化报告，替换现有 R6 手写 eval。

**架构：** 复用现有 `tracer` 采集的 span 轨迹（工具名/参数/顺序/耗时/重试次数），新增 `eval/judge.py`（自研 LLM-as-Judge，用 `call_llm` 打分）+ `eval/trajectory.py`（轨迹指标）+ `eval/report.py`（HTML 报告生成）。runner 升级为采集完整 trajectory 并按三层聚合评分。全程 TDD，flex-fab-agent 现有代码零破坏。
...
| 文件 | 职责 | 动作 |
|------|------|------|
| `flex_fab_agent/eval/__init__.py` | 包导出 | 修改（导出新模块） |
| `flex_fab_agent/eval/metrics.py` | 工具层指标（F1/完整性/订单召回） | 修改（加 min_tools_called 校验 + 微调返回） |
| `flex_fab_agent/eval/judge.py` | **新增** LLM-as-Judge 语义打分（faithfulness/relevancy） | 创建 |
| `flex_fab_agent/eval/trajectory.py` | **新增** 轨迹评估（工具序列/重试/效率/循环） | 创建 |
| `flex_fab_agent/eval/report.py` | **新增** 单页 HTML 报告 | 创建 |
| `flex_fab_agent/eval/runner.py` | 评估运行器（采集 trajectory + 三层聚合） | 修改 |
| `flex_fab_agent/eval/trajectory_capture.py` | **新增** 从 trace + state 重建结构化 trajectory | 创建 |
| `flex_fab_agent/eval/judge_prompt.py` | **新增** judge 提示词模板（LLM-as-Judge） | 创建 |
| `flex_fab_agent/eval/ground_truth.json` | 评估数据集（补 reference/检索上下文） | 修改 |
| `flex_fab_agent/eval/test_metrics.py` | 工具层指标单测 | 创建 |
| `flex_fab_agent/eval/test_trajectory.py` | 轨迹指标单测 | 创建 |
| `flex_fab_agent/eval/test_judge.py` | judge 打分单测（mock LLM） | 创建 |
| `flex_fab_agent/eval/test_runner.py` | runner 三层聚合单测 | 创建 |
| `flex_fab_agent/eval/test_report.py` | 报告生成单测 | 创建 |
| `flex_fab_agent/conftest.py` | pytest 配置 + 路径设置 | 创建 |
| `pytest.ini`（flex-fab-agent 根） | pytest 配置 | 创建 |
| `flex_fab_agent/eval/__init__.py` | 包初始化 | 修改 |

> 说明：`requirements-flex-fab-agent.txt` 无需改动——`pytest` 是开发依赖，不写入容器运行依赖。若需入 CI 再加。
...
**文件：**
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\pytest.ini`
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\conftest.py`

flex-fab-agent 目前零测试、无 pytest 配置。所有后续任务都依赖测试基础设施，这是第一块地基。

> **导入路径关键决策（已验证）**：`eval/judge.py`、`eval/runner.py` 内部用相对导入（`from ..core.llm_client import call_llm`），这要求 `flex-fab-agent` 必须是包（有 `__init__.py`）。因此 conftest 必须把 **agent-training 根**（flex-fab-agent 的父目录）加入 sys.path，测试统一用 `from flex_fab_agent.eval.xxx import ...`。已验证 `python -c "from flex_fab_agent.eval.runner import run_eval"` 在 agent-training 根下成功。

- [ ] **步骤 0：安装 pytest（flex-fab-agent 当前未装）**
...
> 说明：pytest 是开发依赖，不写入 `requirements-flex-fab-agent.txt`（容器运行依赖）。若需 CI 再单列 `requirements-dev.txt`。

- [ ] **步骤 1：创建 pytest.ini**

```ini
# flex_fab_agent 根目录的 pytest 配置
[pytest]
...
- [ ] **步骤 2：创建 conftest.py（把 agent-training 根加入 sys.path，让 `flex-fab-agent` 作为包可导入）**

```python
"""pytest 全局配置：把 agent-training 根加入 sys.path，让 `flex_fab_agent` 包可解析。

eval/ 内部用相对导入（from ..core.llm_client），必须让 `flex_fab_agent` 作为父包导入，
因此这里加的是 agent-training 根（flex_fab_agent 的父目录），不是 flex_fab_agent 根。
"""
import sys
...
> 说明：`flex_fab_agent/pytest.ini` 的 `testpaths = eval tests` 用相对路径，pytest 从 flex-fab-agent 目录启动时自动定位。conftest 放在 flex-fab-agent 根，pytest 收集 eval/ 下测试时自动加载。

- [ ] **步骤 3：创建冒烟测试验证 pytest 能跑**

创建 `C:\workspace\projects\agent-training\flex-fab-agent\eval\test_smoke.py`：
...
def test_demo_package_importable():
    """flex_fab_agent 包能从 agent-training 根导入（验证 conftest 路径设置）。"""
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m pytest eval/test_smoke.py -v`
预期：2 passed

- [ ] **步骤 5：Commit**

```bash
cd C:\workspace\projects\agent-training && git add flex_fab_agent/pytest.ini flex_fab_agent/conftest.py flex_fab_agent/eval/test_smoke.py
...
**文件：**
- 修改：`C:\workspace\projects\agent-training\flex-fab-agent\eval\metrics.py:42-70`
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\test_metrics.py`
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m pytest eval/test_metrics.py -v`
预期：`test_compute_all_metrics_reads_min_tools_called` FAIL（当前不读 min_tools_called），其余 PASS

- [ ] **步骤 3：实现 min_tools_called 校验**

修改 `C:\workspace\projects\agent-training\flex-fab-agent\eval\metrics.py`，在 `compute_all_metrics` 中加入：
...
```bash
cd C:\workspace\projects\agent-training && git add flex_fab_agent/eval/metrics.py flex_fab_agent/eval/test_metrics.py
...
**文件：**
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\judge_prompt.py`
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\judge.py`
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\test_judge.py`
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m pytest eval/test_judge.py -v`
...
def _extract_context(tool_results: list[dict]) -> str:
    ...
    当前 flex_fab_agent 只有 search_knowledge_base 一个 RAG 工具会返回结构化知识片段。
    """
    parts = []
...
```bash
cd C:\workspace\projects\agent-training && git add flex_fab_agent/eval/judge.py flex_fab_agent/eval/judge_prompt.py flex_fab_agent/eval/test_judge.py
...
**文件：**
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\trajectory_capture.py`
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\trajectory.py`
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\test_trajectory.py`
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m pytest eval/test_trajectory.py -v`
预期：全部 PASS

- [ ] **步骤 5：Commit**

```bash
cd C:\workspace\projects\agent-training && git add flex_fab_agent/eval/trajectory.py flex_fab_agent/eval/trajectory_capture.py flex_fab_agent/eval/test_trajectory.py
...
**文件：**
- 修改：`C:\workspace\projects\agent-training\flex-fab-agent\eval\runner.py:27-93`
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\test_runner.py`
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m pytest eval/test_runner.py -v`
预期：FAIL（`_evaluate_single_case` 不存在 → ImportError/AttributeError）

- [ ] **步骤 3：重构 runner，抽出 `_evaluate_single_case` 并集成三层**

修改 `C:\workspace\projects\agent-training\flex-fab-agent\eval\runner.py`：
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m pytest -v`
预期：全部 PASS

- [ ] **步骤 6：Commit**

```bash
cd C:\workspace\projects\agent-training && git add flex_fab_agent/eval/runner.py flex_fab_agent/eval/test_runner.py
...
**文件：**
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\report.py`
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\test_report.py`
...
```python
"""HTML 报告生成测试。"""
from flex_fab_agent.eval.report import render_html_report
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m pytest eval/test_report.py -v`
...
修改 `C:\workspace\projects\agent-training\flex-fab-agent\eval\runner.py` 的 `main`：
...
```bash
cd C:\workspace\projects\agent-training && git add flex_fab_agent/eval/report.py flex_fab_agent/eval/test_report.py flex_fab_agent/eval/runner.py
...
**文件：**
- 修改：`C:\workspace\projects\agent-training\flex-fab-agent\eval\__init__.py`（导出新模块）
- 创建：`C:\workspace\projects\agent-training\flex-fab-agent\eval\test_e2e.py`

最后一个任务：端到端验收。对照用户确认的三个方向（自研 judge / 深做轨迹 / 可视化报告）逐条验证。

- [ ] **步骤 1：更新包导出**

修改 `C:\workspace\projects\agent-training\flex-fab-agent\eval\__init__.py`：
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m pytest eval/test_e2e.py -v`
预期：全部 PASS
...
运行：`cd C:\workspace\projects\agent-training\flex-fab-agent && python -m flex_fab_agent.eval.runner --no-judge`
...
```bash
cd C:\workspace\projects\agent-training && git add flex_fab_agent/eval/__init__.py flex_fab_agent/eval/test_e2e.py
...
- 测试文件 import 路径统一为 `from flex_fab_agent.eval.xxx import ...`（conftest 把 flex-fab-agent 根加入 sys.path）
...
- `requirements-flex-fab-agent.txt` 不添加 pytest（开发依赖，与容器运行依赖分离）——如需 CI 再加
